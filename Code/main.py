...
def recommendation():
    ...
    Returns
    -------
    run the moview recommandation system and visualize 100 movie data
    ...
    for test_num in range(1, 3):
        ...
        print("MAE loss of SVD CF on test %d is equal to %.6f" %(test_num, svd_cf.MAE_loss(predict_matrix, test)))
        visual.visulize_item(predict_matrix, item_data)
        

        # neural net
        predict_matrix2 = neuralnet.neuralnet_predictor(train, user_data, item_data)
        print("RMSE loss of Neural Net on test %d is equal to %.6f" %(test_num, svd_cf.RMSE_loss(predict_matrix2, test)))
        print("MAE loss of SVD CF on test %d is equal to %.6f" %(test_num, svd_cf.MAE_loss(predict_matrix2, test)))

        
        #visual.visulize_item(predict_matrix2, item_data)
